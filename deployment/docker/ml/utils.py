...
from scapy.utils import PcapReader as PcapFileReader
from ml.scapy_aux import PcapReader
...
def read_pcap(filename, pcap_file):
    packets = PcapReader(filename, pcap_file)
    return packets


def read_pcap_file(path: Path):
    packets = PcapFileReader(str(path))
    return packets
